...
@IBDesignable open class SKBatteryView: UIView {
    ...
    @IBInspectable open dynamic var batteryViewBorderColor: UIColor = .foreground {
        didSet {
            bodyOutline.borderColor = batteryViewBorderColor.cgColor
            terminalOutline.borderColor = batteryViewBorderColor.cgColor
            terminalOutline.backgroundColor = batteryViewBorderColor.cgColor
        }
    }

    /// set as 0 for default borderWidth = length / 20
    @IBInspectable open dynamic var batteryViewBorderWidth: CGFloat = 0 { didSet { layoutBattery(); layoutLevel() } }

    /// set as 0 for default cornerRadius = length / 10
    @IBInspectable open dynamic var batteryViewCornerRadius: CGFloat = 0 { didSet { layoutCornerRadius() } }
    ...
    private func layoutBattery() {
        ...
        let terminalLength = terminalLengthRatio * length - 1
        var (terminalFrame, bodyFrame) = bounds.divided(atDistance: terminalLength, from: direction)

        // layout body
        bodyOutline.frame = bodyFrame
        bodyOutline.borderWidth = batteryViewBorderWidth != 0 ? batteryViewBorderWidth : length / 20
...
//        let perpendicularInset: CGFloat = bodyOutline.borderWidth + 1
        let perpendicularInset: CGFloat = -1
    ...
    private func layoutCornerRadius() {
        bodyOutline.cornerRadius = batteryViewCornerRadius != 0 ? batteryViewCornerRadius : length / 10
        terminalOutline.cornerRadius = terminalOutline.frame.height / 2
        if #available(iOS 11.0, *) {
            terminalOutline.maskedCorners = [.layerMaxXMaxYCorner, .layerMaxXMinYCorner]
        } else {
        }
    }
}
